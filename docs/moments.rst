...
The moments can also be saved to FITS files. The `~turbustat.moments.Moments.to_fits` function saves each moment array in the first extension and the uncertainty array in the second. The input to the function is the prefix to use in the filenames::

    >>> mm.to_fits("test")  # doctest: +SKIP

This will produce three FITS files: `test_moment0.fits`, `test_centroid.fits`, `test_linewidth.fits` for the zeroth, first, and square-root of the second moments, respectively.
