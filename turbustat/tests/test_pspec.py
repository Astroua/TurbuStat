...
class testPSpec():
    ...
    def test_PSpec_distance(self):
        self.tester_dist = PSpec_Distance(dataset1, dataset2, fiducial_model = self.tester)
        self.tester_dist = self.tester_dist.distance_metric()
        assert np.allclose(self.tester_dist.distance, self.computed_distances['pspec_distance'])
